...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var options = {
            elementId: "<%= ClientID %>",
            editorType: <%=(int)forum.EditorType%>,
            moduleId: <%=ModuleId%>,
            forumId: <%=ForumId%>,
            ...
            maxUploadSize: <%=forum.AttachMaxSize * 1024%>,
            ...
            allowedFileTypes: "<%= forum.AttachTypeAllowed %>",
            allowBrowseSite: <%= forum.AttachAllowBrowseSite ? 1 : 0 %>,
            attachInsertAllowed: <%= forum.AttachInsertAllowed ? 1 : 0 %>,
            maxFileSizeExceededMsg: "<%=DotNetNuke.UI.Utilities.ClientAPI.GetSafeJSString(string.Format(LocalizeString("[RESX:Error:FileTooLarge]").TextOrEmpty(), forum.AttachMaxSize))%>",
            maxAttachmentCount: <%= forum.AttachCount %>,
